...
class BullohPlotter:
    ...
    @staticmethod
    def _get_chart_title(start_date: datetime, time_setting: TimeChartSettings, chart_type: ChartTypes) -> str:
        """Returns the title of the chart

        Args:
            start_date: Start date of the chart
            time_setting: Time setting of the chart (day, week, or month)
            chart_type (str): Type of the chart, (habits_chart, stats_chart, or time_chart)

        Returns:
            Title with the format {charts_folder}_{time_setting}_{time_setting_number}_{year}_{chart_type}
        """
        match time_setting:
            case TimeChartSettings.DAY:
                date_number = start_date.strftime("%j")
    ...
    def _time_chart_plotter(self, start_date: datetime, end_date: datetime, time_setting: TimeChartSettings)\
        ...
        chart_title = self._get_chart_title(start_date, time_setting, ChartTypes.TIME_CHART)
    ...
    def _get_habits_chart_time_data(df: DataFrame) -> Tuple[dict, int]:
        ...
        for metric in get_time_headers():
            amount_hours = float(round(df[metric].sum(), 1))
            time_metrics[metric] = amount_hours

            if metric == TM.FOCUS_TIME.value:
                amount_hours *= 2

            if metric == TM.LEISURE_TIME.value:
                amount_hours = -amount_hours

            time_points += round(amount_hours)
    ...
    def _stats_chart_plotter(self, start_date: datetime, end_date: datetime, time_setting: TimeChartSettings) -> str:
        logging.info(f"Plotting {time_setting.value} stats chart from {start_date} to {end_date}")
        plot_data = self._list_stats_to_dataframe(self.notion.get_stats_between_dates(start_date, end_date))

        chart_title = self._get_chart_title(start_date, time_setting, ChartTypes.STATS_CHART)
        shutil.copyfile("src/templates/stats_template.html", chart_title)

        time_metrics, time_points = self._get_habits_chart_time_data(plot_data)
        time_metrics["overall_points"] = round(time_points)
    ...
    def plot_charts(self, chart_type: ChartTypes, time_setting: TimeChartSettings) -> List[str]:
        dates = []
        if time_setting == TimeChartSettings.DAY:
            dates = [self.today_date, self.today_date.subtract(days=1), self.today_date.subtract(days=2)]
        elif time_setting == TimeChartSettings.WEEK:
        ...
        for date in dates:
            ...
            elif chart_type == ChartTypes.STATS_CHART:
                file_paths.append(self._stats_chart_plotter(start_date, end_date, time_setting))
